<template>
  <PageActionHeader title="标签" subtitle="浏览和管理标签">
    <template #icon>
  ...
  <div class="container space-y-6 px-6 py-8 relative">
    ...
    <SearchInput
      class="search-input-expand mb-6"
      v-model:search="searchQuery"
      placeholder="请输入作者..."
      :debounce="300"
      @change="paginationRef?.resetPage"
      :min-length="2" />
    ...
    <!-- 分页控制 -->
    <div v-if="totalPages > 1" class="flex flex-1 justify-center">
      <PageNavigator
        ref="paginationRef"
        v-model:current-page="currentPage"
        :page-size="pageSize"
        :total="filteredTags.length"></PageNavigator>
    </div>
...
const filteredTags = computed(() => {
  const validTags = tags.value.filter(
    (tag: Tag) => tag.type !== 'isOriginal' && tag.type !== 'hasSystem',
  )
  ...
  return validTags.filter(
    (tag: Tag) =>
      tag.typeDisplayName.toLowerCase().includes(query) ||
